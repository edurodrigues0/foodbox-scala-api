...
export async function getColaboratorByRegistration(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().get(
    '/collaborators/:registration/registration',
    {
      schema: {
        summary: 'Get Colaborator by Registration',
        tags: ['collaborators'],
        params: z.object({
    ...
    async (request, reply) => {
      try {
        ...
        const colaborator = await db
          ...
          .where(eq(collaborators.registration, registration))
          .limit(1)
          .then((res) => res[0])
